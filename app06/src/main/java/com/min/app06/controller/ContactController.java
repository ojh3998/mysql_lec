...
public class ContactController {
    ...
    // 서비스로부터 연락처 목록과 전체 연락처 갯수를 받아옵니다.

	@RequestMapping(value="/list.do")
	public String list(Model model) {                               // model 은 controller 매개변수에서만 선언가능
	...
	public String detail(HttpServletRequest request, Model model) {
		// 요청 파라미터를 받습니다. Integer.parseInt() 처리를 하고 있으므로 요청파라미터의 null 체크가 필요합니다.
		Optional<String> opt = Optional.ofNullable(request.getParameter("contact_id")); // 있으면 담고 안담아도 오류 안남/.
		int contact_id = Integer.parseInt(opt.orElse("0"));								// 전달이 안되면 "0"을 사용
		// 요청 파라미터를 contact_id로 가지고 있는 연락처 정보를 가져옵니다.
		ContactDto contactDto = contactService.getContact(contact_id);
		// JSP로 전달할 연락처 정보를 Model에 저장
	...
	public String modify(HttpServletRequest request, RedirectAttributes redirectAttributes
		...
		// 요청을 그대로 서비스로 전달하고 수정 성공 or 실패 메시지를 받아옵니다.
		String modifyMsg = contactService.modify(request);
		// 수정 성공/실패 메시지를 RedirctAttributes 에 저장합니다. Model 에 저장하면 redirect할 때 전달 되지  않습니다.
		redirectAttributes.addFlashAttribute("msg", modifyMsg);
		// 연락처 목록 보기로 redirect 합니다. 삽입/수정/삭제/ 이후에는 반드시 redirect 합니다.
	...
	public String remove(HttpServletRequest request, RedirectAttributes redirectAttributes) {
		//요청을 삭제 서비스에 전달하고, 삭제 성공/실패 메세지를 받아옵니다.
		String removeMsg = contactService.reomve(request);
		// 삭제 성공/실패 메시지를 RedirectAttributes 에 저장합니다.
		redirectAttributes.addFlashAttribute("msg", removeMsg);
		//연락처 목록 보기로 redirect합니다.
		return "redirect:/contact/list.do";
	...
	public String register(HttpServletRequest request, RedirectAttributes redirectAttributes ) {
		// 요청을 등록 서비스에 전달한 뒤 등록 성공/실패 메세지를 받아와서 RedirectAttributes 에 저장합니다.
